...
contract Ownable {
    ...
    constructor() internal{
        _owner = msg.sender;
        emit OwnerShip(_owner,"Owner has been set up");
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(!newOwner.isContract(), "the new owner can not be contracts");
        _owner = newOwner;
        emit OwnerShip(_owner,"transfered the ownership");
    }
    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    event OwnerShip(address indexed owner, string indexed message);
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        // TODO require the given address to not be the owner of the tokenId
            require(ownerOf(tokenId) != to, "Can not be the owner of the tokenId");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
            require(msg.sender == owner() || isApprovedForAll(ownerOf(tokenId),msg.sender), "cloud not authorize");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        if(_exists(tokenId))
        {
            revert("token exists already");
        }
        if(to.isContract())
        {
            revert("can not mint token for invalid address");
        }
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        // TODO: require from address is the owner of the given token
            require(_tokenOwner[tokenId] == from, "Can not transfer token you do not own");
        // TODO: require token is being transfered to valid address
            require(!to.isContract(), "can not transfer token for invalid address");
        // TODO: clear approval
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId),"Given token does not exists");
    ...
    {
        require(_exists(tokenId),"Can not set URl to token that does not exists");
...
//      -calls the superclass mint and setTokenURI functions
    ...
    function mint(address to, uint tokenId, string memory tokenURI) public
    onlyOwner
    returns(bool)
    {
        super._mint(to,tokenId);
